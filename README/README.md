...
<img width="1469" height="870" alt="스크린샷 2026-08-10 오전 2 09 04" src="https://github.com/user-attachments/assets/867de4a0-9223-407b-839d-eeccc5e73294" />
...
## ERD
<img width="644" height="618" alt="스크린샷 2026-08-10 오전 2 25 34" src="https://github.com/user-attachments/assets/f5112cd0-bd64-4b4b-a8f8-e61df5c1f6ec" />
...
```mermaid
flowchart LR
    Client[Web Client] -->|REST API / JWT| API[Spring Boot API]
    API --> Auth[Authentication Filter]
    Auth --> Domain[Domain Services]
    Domain --> JPA[JPA + QueryDSL]
    JPA --> DB[(MySQL)]
    Domain -->|Presigned URL| S3[(AWS S3)]
...
CI는 Pull Request와 `dev` 브랜치 push 시 Docker 이미지를 빌드하고 컨테이너 헬스 체크를 수행합니다. 
`dev` 이미지 배포는 GitHub Actions에서 Docker Hub로 이미지를 푸시한 뒤, Argo CD가 Helm 설정 변경을 감지하는 GitOps 방식으로 구성되어 있습니다.
